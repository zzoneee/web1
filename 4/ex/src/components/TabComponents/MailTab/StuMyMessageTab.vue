    ...
    <div class="myMessageContainer">
        ...
        <el-dialog customClass="searchContactDiag" title="查找联系人" :visible.sync="dialogFormVisible"  @closed="handleClose" :append-to-body="true">
            ...
            <div class="recentContactSearch" v-if="searchContactForm.identity!=2&&searchContactForm.identity!=4">
                <el-tag type="success" v-if="searchContactForm.identity==0" style="margin-top: 9px; margin-left:11px;">教师</el-tag>
                <el-tag type="primary" v-if="searchContactForm.identity==1||searchContactForm.identity==3" style="margin-top: 9px; margin-left:11px;">学生</el-tag>
...
<script>
    ...
    export function get_stuGetPrivateLetterLists() {
        return axios({
            method: 'get',
            url: "/api/privateLetter/stuGetPrivateLetterLists",
            headers: {'Authorization': window.localStorage['Authorization']}
        })
    }
    export function post_stuEnterPrivateLetter(receiver, message, identity) {
        return axios({
            method: 'post',
            url: "/api/privateLetter/stuEnterPrivateLetter",
            headers: {'Authorization': window.localStorage['Authorization']},
            data:{receiver: receiver, message: message, identity: identity}
        })
    }
    export function get_stuRecentContacts() {
        return axios({
            method: 'get',
            url: "/api/privateLetter/stuRecentContacts",
            headers: {'Authorization': window.localStorage['Authorization']}
        })
    }
    export function post_stuCloseChatBox(receiver, iden) {
        return axios({
            method: 'post',
            url: "/api/privateLetter/stuCloseChatBox",
            headers: {'Authorization': window.localStorage['Authorization']},
            data:{receiver: receiver, iden: iden}
        })
    }
    export function post_stuOpenChatBox(receiver, identity) {
        return axios({
            method: 'post',
            url: "/api/privateLetter/stuOpenChatBox",
            headers: {'Authorization': window.localStorage['Authorization']},
            data:{receiver: receiver, identity: identity}
        })
    }
    export function get_stuSearchContact(receiver, identity) {
        return axios({
            method: 'get',
            url: "/api/privateLetter/stuSearchContact",
            headers: {'Authorization': window.localStorage['Authorization']},
    ...
    export default {
        ...
        methods:{
            ...
            initGetMsg(){
                get_stuGetPrivateLetterLists().then(res => {
            ...
            removeTab(targetName) {
                ...
                post_stuCloseChatBox(receiver, iden).then(res => {
            ...
            enterMsgClick(identity){
                if(this.enterMsgs != ''){
                    ...
                    post_stuEnterPrivateLetter(receiver, this.enterMsgs, identity).then(res => {
                        if(res.data.status == 200){
                            // console.log(res.data.data);
                            // this.noticeLists = res.data.data
                            this.initGetMsg();// 刷新信息
            ...
            openDrawer(){
                this.drawer = true;
                get_stuRecentContacts().then(res => {
                    // console.log(res.data.status);
                    if(res.data.status == 200){
                        // console.log(res.data.data);
            ...
            searchContact(receiver, identity){
                if(receiver != '' && receiver != null){
                    // console.log(receiver + " " + identity);
                    get_stuSearchContact(receiver, identity).then(res => {
                        // console.log(res.data.status);
                        if(res.data.status == 200){
                            // console.log(res.data.data);
