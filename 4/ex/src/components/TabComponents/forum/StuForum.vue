...
<script>
    ...
    export function get_stuPostMsg(st, ed, PostOptionsValue) {
        return axios({
            method: 'get',
            url: "/api/posts/stuPostMsg",
            headers: {'Authorization': window.localStorage['Authorization']},
            params: {st: st, ed: ed, PostOptionsValue: PostOptionsValue}
        })
    }
    export function post_stuReleasePost(title, message) {
        return axios({
            method: 'post',
            url: "/api/posts/stuReleasePost",
            headers: {'Authorization': window.localStorage['Authorization']},
    ...
    export function post_stuCommentPost(id, message) {
        return axios({
            method: 'post',
            url: "/api/posts/stuCommentPost",
            headers: {'Authorization': window.localStorage['Authorization']},
    ...
    export function post_stuThumbsUp(id) {
        return axios({
            method: 'post',
            url: "/api/posts/stuThumbsUp",
            headers: {'Authorization': window.localStorage['Authorization']},
            data: {id: id}
        })
    }
    export function post_stuThumbsDown(id) {
        return axios({
            method: 'post',
            url: "/api/posts/stuThumbsDown",
            headers: {'Authorization': window.localStorage['Authorization']},
            data: {id: id}
        })
    }
    export default {
        ...
        methods:{
            ...
            getPostLists(st, ed, flag = false){
                if(flag){
                    get_stuPostMsg(st, ed, this.PostOptionsValue).then(res => {
            ...
            handleSave () {
                this.$refs.ruleForm.validate((valid) => {
                    if (valid) {
                        ...
                        post_stuReleasePost(this.form.title, this.form.message).then(res => {
                            if(res.data.status == 200){
            ...
            commenthandleSave () {
                this.$refs.commentruleForm.validate((valid) => {
                    if (valid) {
                        // this.$message.success("发布成功");
                        // console.log(this.form.title, this.form.message);
                        post_stuCommentPost(this.drawerPost.id, this.commentform.message).then(res => {
                            if(res.data.status == 200){
            ...
            // 点赞
            thumbsUp(post){
                post_stuThumbsUp(post.id).then(res => {
                    if(res.data.status == 200){
                        this.getPostLists((this.pageNo - 1) * this.pageSize + 1, this.pageNo * this.pageSize, true);
                        if(this.drawer){
            ...
            // 取消赞
            thumbsDown(post){
                post_stuThumbsDown(post.id).then(res => {
                    if(res.data.status == 200){
                        this.getPostLists((this.pageNo - 1) * this.pageSize + 1, this.pageNo * this.pageSize, true);
                        if(this.drawer){
...
<style scoped>
    ...
    .btn_setting{
        background-image: url("../../../assets/btn_forum.jpg");
        float:left;
        margin-left: 155px;
        width: 145px;
        height: 38px;
